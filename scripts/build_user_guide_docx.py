"""Build docs/User-Guide.docx from docs/User-Guide.md."""
...
MD_PATH = Path(__file__).resolve().parents[1] / "docs" / "User-Guide.md"
OUT_PATH = Path(__file__).resolve().parents[1] / "docs" / "User-Guide.docx"
...
def clear_metadata(doc: Document):
    ...
    cp.comments = ""
    cp.subject = "Charging simulator user guide"
    cp.keywords = "Massive Mobility, OCPP, simulator, user guide, EVSE"
    cp.category = "User documentation"
    cp.title = "Massive Mobility Charging Simulator — User Guide"
...
def build_document(md_text: str) -> Document:
    ...
    for raw in md_text.splitlines():
        ...
        if line.startswith("# "):
            ...
            p = doc.add_paragraph(strip_md(re.sub(r"^\d+\.\s+", "", line)), style="List Number")
            for run in p.runs:
...
def main():
    ...
    try:
        ...
        fallback = OUT_PATH.with_name(OUT_PATH.stem + "-new.docx")
        doc.save(fallback)
        print("wrote", fallback, "(close Word if the main file is open)")
